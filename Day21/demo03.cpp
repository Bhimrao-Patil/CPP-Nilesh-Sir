...
int main() {
	...
	/*
		int x, y, z = 10;
		x = y = z; // here, assignment operator is used, so we need to overload assignment operator for our class to avoid shallow copy
		println(x); // 10
		println(y); // 10
		println(z); // 10
	*/
	d4 = d3 = d1;// d4.operator=(d3.operator=(d1)) // here, assignment operator is used, so we need to overload assignment operator for our class to avoid shallow copy
	// but it will give an error if assignment operator is not overloaded
	// soltion: return *this; in assignment operator, so that we can chain the assignment operator
	cout << "d4 => " << d4 << endl;
	return 0;
}
/*
